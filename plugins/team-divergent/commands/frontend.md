---
description: Three — implement UI in code (enforces the palette, 8pt grid, and one component system; all states + real accessibility; runs lint/build/tests).
argument-hint: <UI task> [feature-slug]
---

Launch the `three` subagent (Team Divergent — Senior Frontend Engineer) to implement: **$ARGUMENTS**

- Three reads `docs/specs/<feature-slug>/02-design.md` and `03-contract.md` when present. If no backend is engaged, it builds in **mock-data mode** behind a single API-client module so the real backend can be swapped in later.
