...
4. Execute the waves in order: launch each wave's members **in parallel** via the Task tool, relay each member's summary, then proceed. Pause for approval at any gate wave (Rin review, Candor QA) or if any member reports a blocker.
5. If a member logs a blocker in `docs/specs/<feature-slug>/blockers.md`, re-launch `kin` to resolve it before continuing.

Never soften Candor's **FAIL** or Rin's **CHANGES REQUIRED** verdicts. Subagent prompts must be self-contained: they cannot see this conversation.
