---
description: Three — replace raw/random colors with harmony-based palette tokens (Color Wheel Theory), fix contrast failures, and remove unmotivated gradients.
argument-hint: [file, component, or page — defaults to recent UI changes]
---

Launch the `three` subagent (Team Divergent — Senior Frontend Engineer) to fix colors on: **$ARGUMENTS** (default: recently changed UI files).
