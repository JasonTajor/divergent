---
description: Abnegation — find and fix backend performance problems — N+1 queries, missing indexes, SELECT *, unbounded queries, row-by-row loops.
argument-hint: [endpoint, query, module, or file — defaults to recent backend changes]
---

Launch the `abnegation` subagent (Team Divergent — Senior Backend Developer & Architect) to optimize: **$ARGUMENTS** (default: recently changed backend code).
