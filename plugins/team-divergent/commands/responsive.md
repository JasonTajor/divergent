---
description: Dauntless — make a page/component fully responsive (mobile-first) across mobile/tablet/desktop on the 8pt grid, with no overflow or horizontal scroll.
argument-hint: [file, component, or page — defaults to recent UI changes]
---

Launch the `dauntless` subagent (Team Divergent — Senior Frontend Engineer) to make responsive: **$ARGUMENTS** (if nothing is named, target the most recently changed UI files).
