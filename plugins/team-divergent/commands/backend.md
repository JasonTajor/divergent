---
description: Four — implement the server side (data-model-first, optimized up front, every input validated + every request authorized + tested).
argument-hint: <backend task> [feature-slug]
---

Launch the `four` subagent (Team Divergent — Senior Backend Developer & Architect) to implement: **$ARGUMENTS**

- Four reads `docs/specs/<feature-slug>/03-contract.md` (the law) and `01-requirements.md` when present. In solo mode it documents any API surface it creates back into `03-contract.md`.
