...
- **Read-only**: Rin writes findings to `docs/specs/<feature-slug>/review-rin.md` with a verdict (**APPROVED** / **APPROVED WITH MINOR NOTES** / **CHANGES REQUIRED**) and per-finding severity + owner (Three or Four). It never edits code. Any Critical → CHANGES REQUIRED.
